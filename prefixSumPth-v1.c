                           ...
                           int nThreads )
{
	...
	int start = 0;
    int partes = nTotalElmts / nThreads;
    int resto = nTotalElmts % nThreads;

	//pool threads cada thread executa uma parte da entrada
    for (int i = 0; i < nThreads; i++){
        ...
        pthread_create(&Thread[i], NULL, threadPrefixSum, (void*)&threadData[i]);
    ...
    // Calcular soma acumulada dos prefixos parciais
    TYPE totalSum = 0;
    for (int i = 0; i < nThreads; i++){
        totalSum += threadData[i].sum;
        threadData[i].sum = totalSum;
    }

    // Ajustar os valores finais do vetor de saída
    for (int i = 1; i < nThreads; i++){
        int start = threadData[i].start;
        int end = threadData[i].end;

        TYPE ajuste = threadData[i - 1].sum;

        for (int j = start; j < end; j++)
            OutputVector[j] += ajuste;
    }
}
